...
$$
u' = f,\qquad u'' = Df,\qquad u''' = D^2 f.
$$
La expansión de Taylor de $u$ es
$$
u(x_j+h)=u_j + h f_j + \frac{h^2}{2}\,D f_j + \frac{h^3}{6}\,D^2 f_j + \mathcal{O}(h^4).
$$
- Primera etapa:
$$
k_1=f_j.
...
$$
k_2=f\!\left(x_j+\tfrac{h}{2},\,u_j+\tfrac{h}{2}k_1\right)
   = f_j + \tfrac{h}{2}\,D f_j + \tfrac{h^2}{8}\,D^2 f_j + \mathcal{O}(h^3).
$$

- Tercera etapa (usando la misma técnica y reteniendo términos hasta $h^2$):
$$
k_3=f\!\left(x_j+h,\,u_j-hk_1+2h\,k_2\right)
   = f_j + h\,D f_j + \tfrac{h^2}{2}\,D^2 f_j + \mathcal{O}(h^3).
$$
...
= f_j + \tfrac{h}{2}\,D f_j + \tfrac{h^2}{6}\,D^2 f_j + \mathcal{O}(h^3).
$$
...
u_{j+1}
= u_j + h\,\Phi_h(x_j,u_j)
= u_j + h f_j + \tfrac{h^2}{2}\,D f_j + \tfrac{h^3}{6}\,D^2 f_j + \mathcal{O}(h^4),
$$
que coincide con la expansión de $u(x_j+h)$ hasta términos $\mathcal{O}(h^3)$.

Desta forma se puede concluir
