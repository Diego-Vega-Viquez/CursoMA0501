...
```
Para que se obtener la precision pedida, se requieren de teoreticamente mas de :
```{r}
round(5* log(10) / log(2))
```
iteraciones, lo cual es consistente con el resultado del codigo anterior
...
sprintf("%.20f", raiz.b)
```
Para obtener la precision pedida, se requieren, teoreticamente, de las mismas iteraciones que el inciso anterior (pues la longitud del intervalo es la misma), lo cual es consistente con lo obtenido ahora.
...
$$
f(x) = e^{-x}, \quad \Rightarrow f'(x) = -e^{-x}
$$

  Se desea encontrar un intervalo $[a,b]$ donde $f$ sea contractiva. Como $|f'(x)| = e^{-x}$, se cumple que:

$$
|f'(x)| < 1 \Rightarrow x > 0
$$

  Entonces $[0,1]$ es un intervalo que se mapea en sí mismo a través de $f$, y cumple que:

$$
q := \sup |f'(x)| = e^{-0.5} \approx 0.6065
$$

  El número de iteraciones necesarias para asegurar un error absoluto menor a $10^{-5}$ es:

\begin{align*}
n &\geq \ln \left( \frac{10^{-5}(1 - e^{-0.5})}{e^{-0.5}(0.5)} \right) \cdot \frac{1}{\ln(e^{-0.5})} \\
&\approx 20.71 \\
\Rightarrow n &\geq 21 \text{ iteraciones}
\end{align*}
...
  Se quiere que $|f'(x)| \leq 1$, entonces:

\begin{align*}
\frac{e^x}{3} &\leq 1 \\
e^x &\leq 3 \\
x &\leq \ln(3) \approx 1.09
\end{align*}

  Entonces el intervalo $[0,1]$ cumple con que:

- Se mapea en sí mismo a través de $f$
- Se cumple que $q = \dfrac{e^{0.7}}{3} < 1$

  Estimamos el número de iteraciones:

\begin{align*}
n &\geq \ln\left( \frac{10^{-5}(1 - \frac{e^{0.7}}{3})}{\frac{e^{0.7}}{3}(0.6)} \right) \cdot \frac{1}{\ln(\frac{e^{0.7}}{3})} \\
&\approx 19.35 \\
\Rightarrow n &\geq 20 \text{ iteraciones}
\end{align*}

::: {.callout-note}
###### Notación

El error en la iteración de punto fijo puede acotarse mediante:

$$
|x_n - x| \leq \frac{q^n}{1 - q} |x_1 - x_0|
$$

donde $q$ es la constante de Lipschitz (cota del valor absoluto de la derivada en el intervalo).
...
$$
x = g(x) = \left(\frac{\sqrt{3}}{3}\right) e^{x/2}
$$

  Derivada:
$$
g'(x) = \left(\frac{\sqrt{3}}{6}\right) e^{x/2}
$$

  En el intervalo $[0,1]$:
$$
\sup_{x \in [0,1]} |g'(x)| \approx \frac{1.732}{6} \cdot e^{0.5} \approx 0.475 < 1
$$
...
  Por tanto, $g([0,1]) \subseteq [0.577, 0.950] \subseteq [0,1]$
$$\therefore \text{Se cumple el teorema de Banach}$$
```{r}
...
b)La ecuación puede reescribirse como:
$$
x = g(x) = \cos(x)
$$
...
  En el intervalo $[0,1]$, se tiene:
$$
\sup_{x \in [0,1]} |g'(x)| = \max_{x \in [0,1]} |\sin(x)| = \sin(1) \approx 0.841 < 1
$$

  Además, evaluamos extremos:

- $g(0) = \cos(0) = 1$
- $g(1) = \cos(1) \approx 0.5403$

  Entonces:
$$
g([0,1]) \subseteq [0.5403, 1] \subseteq [0,1]
$$

  Se cumple que $g$ es una contracción y $g([0,1]) \subseteq [0,1]$.  
  Por tanto, por el teorema de Banach, la función tiene un único punto fijo en $[0,1]$, y la iteración converge.
...
Considere la ecuación $x^2 - x - 2 = 0$.

a) Encuentre una función $g(x)$ que permita resolver esta ecuación en el intervalo $[0, 7]$ utilizando el algoritmo del **Punto Fijo**. Debe probarse que $g(x)$ cumple la hipótesis del teorema visto en clase.

b) Determine usando el estimado del error a priori y a posteriori cuántas iteraciones se requieren para que el error absoluto sea menor a $\epsilon = 10^{-6}$ si se toma $p_0 = 0$.
...
  Una posible reescritura para usar punto fijo es:

$$
x = g(x) = \sqrt{x + 2}
$$

  Esta forma es válida porque:

$$
x = \sqrt{x + 2} \Rightarrow x^2 = x + 2 \Rightarrow x^2 - x - 2 = 0
$$
...
  En el intervalo $[2, 3]$, se tiene:

- $\sqrt{2 + 2} = 2$ ⇒ $g'(x) \leq \frac{1}{4} = 0.25 < 1$

  Evaluación:

- $g(2) = \sqrt{4} = 2$
- $g(3) = \sqrt{5} \approx 2.236$

  Entonces:
$$
g([2, 3]) \subseteq [2, 2.236] \subseteq [2, 3]
$$

$g$ es continua, contractiva y mapea el intervalo sobre sí mismo. Se cumple el teorema de Banach.
```{r}
# Definición de g(x)
g <- function(x) sqrt(x + 2)
# Ejecución con p0 = 2.0
punto.fijo(0, 1e-6, 100, g)
```

b) Sabemos que el error a priori está acotado por:

$$
|p_n - p| \leq \frac{L^n}{1 - L} |p_1 - p_0|
$$

Donde $L = \sup |g’(x)|$ en el intervalo $[2, 3]$:

$$
g’(x) \leq \frac{1}{2 \sqrt{4}} = \frac{1}{4} = 0.25
$$
```{r}
# Estimación a priori
...
# Fórmula del error a priori
n_apriori <- ceiling(log((epsilon * (1 - L)) / abs(p1 - p0)) / log(L))
n_apriori

# Estimación a posteriori

# Usando última iteración:
# p_n = último valor aproximado
# p_{n-1} = valor anterior

# Supongamos:
p_n <- g(g(g(g(g(p0)))))       # aproximación tras 5 iteraciones
p_n1 <- g(g(g(g(p0))))         # aproximación tras 4 iteraciones

# Cota a posteriori:
error_post <- (L / (1 - L)) * abs(p_n - p_n1)
error_post
```
...
Dado que $p$ es un cero de multiplicidad $m$, entonces podemos escribir:

$$
f(x) = (x - p)^m q(x)
$$

donde $q(x)$ es una función continua y $q(p) \ne 0$.

Ahora derivamos $f(x)$ usando la regla del producto:

\begin{align*}
f'(x) &= \frac{d}{dx}\left[ (x - p)^m q(x) \right] \\
&= m(x - p)^{m-1} q(x) + (x - p)^m q'(x)
\end{align*}

Sustituimos $f(x)$ y $f'(x)$ en $g(x)$:

\begin{align*}
g(x) &= x - m \frac{(x - p)^m q(x)}{m(x - p)^{m-1} q(x) + (x - p)^m q'(x)} \\
&= x - m \cdot \frac{(x - p)^m q(x)}{(x - p)^{m-1} \left[ m q(x) + (x - p) q'(x) \right]} \\
&= x - \frac{(x - p)^m q(x)}{(x - p)^{m-1} \left[ q(x) m + (x - p) q'(x) \right]} \\
&= x - \frac{(x - p) q(x)}{q(x) m + (x - p) q'(x)}
\end{align*}

Llamemos a la última expresión $g(x) = x - h(x)$, con:

$$
h(x) = \frac{(x - p) q(x)}{q(x) m + (x - p) q'(x)}
$$
...
Para calcular $h'(x)$, usamos la regla del cociente:

\begin{align*}
h'(x) &= \frac{[(x - p) q(x)]' \cdot [q(x)m + (x - p) q'(x)] - (x - p) q(x) \cdot [q(x)m + (x - p) q'(x)]'}{[q(x)m + (x - p) q'(x)]^2}
\end{align*}

Pero en realidad no necesitamos derivar todo: evaluamos directamente $g'(x)$ en $x = p$.

Como $x \to p$:

\begin{align*}
h(x) &= \frac{(x - p) q(x)}{q(x) m + (x - p) q'(x)} \to \frac{0}{q(p)m + 0} = 0
\end{align*}

Y usando L’Hôpital:

\begin{align*}
\lim_{x \to p} h(x) &= \lim_{x \to p} \frac{(x - p) q(x)}{q(x) m + (x - p) q'(x)} \\
&= \frac{0}{m q(p)} = 0 \\
\Rightarrow h(p) &= 0 \\
\Rightarrow g(p) &= p
\end{align*}
...
\begin{align*}
g'(x) &= 1 - h'(x) \Rightarrow g'(p) = 1 - h'(p)
\end{align*}

Pero, como $h(x) \sim (x - p)$ para $x$ cercano a $p$, entonces $h'(p) = 1$ y por tanto:
...
$$
x_{n+1} = x_n - \frac{x_n^m - R}{m x_n^{m-1}} = x_n - \frac{x_n}{m} + \frac{R}{m x_n^{m-1}}
$$

Por lo tanto:

$$
x_{n+1} = \left(1 - \frac{1}{m} \right)x_n + \frac{R}{m x_n^{m-1}}
$$

---

Caso 2: $f(x) = 1 - \dfrac{R}{x^m}$
...
$$
x_{n+1} = x_n - \frac{f(x_n)}{f'(x_n)} = x_n - \left(1 - \frac{R}{x_n^m} \right) \cdot \frac{x_n^{m+1}}{mR}
$$

Observamos que:

\begin{align*}
x_{n+1} &= x_n - \frac{x_n^{m+1} - x_n R}{mR} \\
        &= x_n - \frac{x_n^{m+1}}{mR} + \frac{x_n}{m}
\end{align*}

Por lo tanto:

$$
x_{n+1} = \left(1 + \frac{1}{m} \right)x_n - \frac{x_n^{m+1}}{m R}
$$
...
La diferencia clave entre ambas fórmulas está en el segundo término:

- En la **segunda fórmula** aparece $\dfrac{x^{m+1}}{mR}$, que puede crecer enormemente si $x$ o $m$ son grandes. Esto puede causar **sobreflujo** o pérdida de precisión numérica en las computadoras.

- En cambio, en la **primera fórmula** se usa $\dfrac{R}{m x^{m-1}}$, el cual tiende a hacerse **más pequeño conforme $x$ crece**, lo que ayuda a **estabilizar la iteración**.

Por ello, **la primera opción es mucho más segura numéricamente**.
:::

**¿Cuál fórmula es mejor?**  
Para $R > 0$ y $m \geq 1$, la **primera fórmula**:

$$
x_{n+1} = \left(1 - \frac{1}{m} \right)x_n + \frac{R}{m x_n^{m-1}}
$$

es generalmente más estable y eficiente, especialmente cuando se parte de una buena aproximación inicial $x_0$ lejos de cero.
...
**a.** El método **Regula Falsi** consiste en **trazar una línea recta entre los puntos extremos del intervalo $[a_i, b_i]$ sobre la curva de $f$** y hallar el punto en que esta recta intersecta el eje $x$. Ese punto se usa como una mejor aproximación a la raíz.

- Geométricamente, se asemeja al método de la **secante**, pero manteniendo la propiedad de que la raíz siempre queda contenida entre los extremos del intervalo.
- Luego se actualiza el intervalo según el signo de $f(p_i)$.

**b.** Ecuación de la recta que pasa por $(a_i, f(a_i))$ y $(b_i, f(b_i))$

La recta que pasa por dos puntos $(x_1, y_1)$ y $(x_2, y_2)$ tiene la forma:
...
**c.** Buscamos el punto de intersección con el eje $x$ (i.e., donde $y = 0$):

\begin{align*}
0 - f(a_i) &= \frac{f(b_i) - f(a_i)}{b_i - a_i}(p_i - a_i) \\
\Rightarrow -f(a_i)(b_i - a_i) &= (f(b_i) - f(a_i))(p_i - a_i) \\
\Rightarrow p_i - a_i &= \frac{-f(a_i)(b_i - a_i)}{f(b_i) - f(a_i)} \\
\Rightarrow p_i &= a_i - \frac{f(a_i)(b_i - a_i)}{f(b_i) - f(a_i)}
\end{align*}

Multiplicando y dividiendo, obtenemos una forma simétrica:

$$
p_i = \frac{a_i f(b_i) - b_i f(a_i)}{f(b_i) - f(a_i)}
$$

**d)** Pseudocódigo del método Regula Falsi

```{pseudo}
Entrada: f, a, b, tol, Nmax

Si f(a) * f(b) ≥ 0 entonces
    Detener: no se garantiza una raíz en [a, b]
Fin si

Para i desde 1 hasta Nmax hacer:
    Calcular:
...
Retornar error: no se alcanzó la tolerancia deseada
```

**e)** Implementación en R
...
# Leyenda
legend("topleft",
       legend = c("f1(x) = log(1+x) - x^2",
                  "f2(x) = x^3 + 2x - 1"),
       col = c("blue", "red"), lwd = 2)
```
A partir del gráfico se puede observar que f1 interseca con el eje x en dos puntos: uno parece estar entre -0.5 y 0.5, mientras el otro parece estar entre0.5 y 1.

Por otra parte, f2 parece intersecar entre 0 y 1
a)
```{r}
...
Note que calcular $\sqrt[m]{a}$ es equivalente a resolver la ecuación:

$$
x^m - a = 0
$$

Aplicando el método de Newton–Raphson con $f(x) = x^m - a$, tenemos:

$$
f'(x) = m x^{m-1}
$$

Entonces, la fórmula de Newton–Raphson es:

\begin{align*}
x_{n+1} &= x_n - \frac{x_n^m - a}{m x_n^{m-1}} \\
        &= \frac{m x_n^m - (x_n^m - a)}{m x_n^{m-1}} \\
        &= \frac{a + (m - 1)x_n^m}{m x_n^{m-1}}
\end{align*}

Esto coincide exactamente con la fórmula de recurrencia dada para $n \geq 1$.

Por lo tanto, como la sucesión está construida con el método de Newton–Raphson para una función que cumple las condiciones del teorema de convergencia de Newton, sabemos que esta sucesión converge a la solución de $x^m - a = 0$, la cual es:
...
Sea $x_n = \dfrac{1}{n^k}$ para $k \in \mathbb{Z}^+$.

Queremos verificar si la sucesión converge **linealmente** a $x = 0$.

Calculamos:

$$
\lim_{n \to \infty} \frac{|x_{n+1} - x|}{|x_n - x|} = \lim_{n \to \infty} \frac{\frac{1}{(n+1)^k}}{\frac{1}{n^k}} = \lim_{n \to \infty} \left(\frac{n}{n+1}\right)^k = 1
$$

Por lo tanto, $x_n$ converge linealmente a $0$ con constante asintótica $\lambda = 1$ (o más estrictamente, con $\lambda \lesssim 1$ pero **no estrictamente menor** que 1).

Como el cociente tiende a 1, se puede decir que la convergencia es **sub-lineal** si se requiere $\lambda < 1$, pero para fines prácticos de velocidad de convergencia, se considera **lineal lenta**.

Sean $k, m \in \mathbb{Z}^+$, queremos:
...
N^k > 10^m
\quad \Leftrightarrow \quad
N > \sqrt[k]{10^m} = 10^{\frac{m}{k}}
$$

Por lo tanto, basta tomar:

$$
N = \left\lceil 10^{\frac{m}{k}} \right\rceil
$$

donde $\lceil \cdot \rceil$ denota el **techo entero** (menor entero mayor o igual que el número).
...
Empezamos por los términos de la sucesión: $$
x_n = \frac{1}{n}, \quad x_{n+1} = \frac{1}{n+1}, \quad x_{n+2} = \frac{1}{n+2}
$$

Calculamos sus diferencias: $$
\Delta x_n = -\frac{1}{n(n+1)}, 
\qquad
\Delta^2 x_n = \frac{2}{n(n+1)(n+2)}
$$

La Transformada resulta en: $$
\bar{x}_n 
= \frac{1}{n} - \frac{\left(-\tfrac{1}{n(n+1)}\right)^2}{\tfrac{2}{n(n+1)(n+2)}}
= \frac{1}{2(n+1)}
$$

Verificando la condición del error: $$
\left|\bar{x}_n\right| = \frac{1}{2(n+1)} \leq 0.05
\quad \Rightarrow \quad n \geq 9
$$
...
Empezamos por los términos de la sucesión: $$
x_n = \frac{1}{n^2}, \quad x_{n+1} = \frac{1}{(n+1)^2}, \quad x_{n+2} = \frac{1}{(n+2)^2}
$$

Tras un proceso similar al ejercicio anterior, la simplificación para $\bar{x}$: $$
\bar{x}_n = \frac{1}{(n+1)(n+2)}.
$$

Verificando la condición: $$
\left|\bar{x}_n\right| = \frac{1}{(n+1)(n+2)} \leq 0.05.
$$

Si evaluamos en $n=3$: $$
\bar{x}_3 = \frac{1}{4 \cdot 5} = 0.05.
$$

Por lo que cumple desde $n=3$.
...
d) ¿Cuántas iteraciones se requieren teóricamente para minimizar la función $f(x) = x^4 - 4x^3 + 3x^2$ en el intervalo $[1, 4]$ si se desea un error absoluto menor a $10^{-5}$? Encuentre el mínimo.
:::
...
\begin{align*}
&\Rightarrow 2a < b + a \\
&\Rightarrow 2a + b < 2b + a \\
&\Rightarrow \frac{2a + b}{3} < \frac{2b + a}{3} \\
&\Rightarrow x_1 < x_2
\end{align*}
...
c) Queremos probar que el intervalo construido con el algoritmo anterior satisface:
...
\begin{align*}
|b_1 - a_1| &= |b - a| \\
|b_2 - a_2| &= \left(\frac{2}{3}\right)|b - a| \\
|b_3 - a_3| &= \left(\frac{2}{3}\right)^2 |b - a| \\
&\vdots \\
|b_n - a_n| &= \left(\frac{2}{3}\right)^{n-1}|b - a|
\end{align*}

Como $x_n \in ]a_n, b_n[$ y $x_n = \frac{2a_n + b_n}{3}$ o $x_n = \frac{a_n + 2b_n}{3}$

Entonces:
\begin{align*}
|x_n - x| &\leq \frac{2}{3}|b_n - a_n| \leq \frac{2}{3} \left(\frac{2}{3}\right)^{n-1} |b - a| \\
&= \left(\frac{2}{3}\right)^n |b - a|
\end{align*}
...
$$
|x_n - x| \leq \left(\frac{2}{3}\right)^n \cdot 3 < 10^{-5}
$$

\begin{align*}
\left(\frac{2}{3}\right)^n &< 10^{-5} \cdot 3 \\
n &> \frac{\ln(10^{-5} \cdot 3)}{\ln\left(\frac{2}{3}\right)} \approx 31.10 \\
\Rightarrow n &\geq 32 \text{ iteraciones}
\end{align*}

El valor mínimo de la función se obtiene de la siguiente manera.
```{r}
...
min.x <- dicotomico(1, 4, 1e-6, 1e-5, 100, f)
min.x
```
Para encontrar el valor minimo de la funcion en [1,4], esta se evalua en el punto encontrado recientemente:
```{r}
f(min.x$valor)
```
...
a) Se va a probar por inducción que $r_n \leq (r_0)^{2^n}$.

  **Caso base**: $n = 1$.

  Se tiene que $r_1 \leq r_0^2$, lo cual es cierto porque la sucesión cumple que:

$$
r_{n+1} \leq r_n^2
$$

  **Paso inductivo**: Suponga que $r_n \leq (r_0)^{2^n}$. Queremos probar que $r_{n+1} \leq (r_0)^{2^{n+1}}$.

  Por la hipótesis inductiva:

\begin{align*}
r_n &\leq (r_0)^{2^n} \\
\Rightarrow r_n^2 &\leq \left( (r_0)^{2^n} \right)^2 = (r_0)^{2^{n+1}}
\end{align*}

  Y dado que $r_{n+1} \leq r_n^2$, entonces:

$$
r_{n+1} \leq (r_0)^{2^{n+1}} \quad \blacksquare
$$

  **Para demostrar convergencia a $0$**, observamos que si $0 < r_0 < 1$, entonces:

$$
(r_0)^{2^n} \to 0 \quad \text{cuando } n \to \infty
$$

  y como $0 \leq r_n \leq (r_0)^{2^n}$, se concluye que:

$$
r_n \to 0 \quad \text{cuando } n \to \infty \quad \blacksquare
$$

b. Queremos demostrar:

$$
x_{n+1} - x = \frac{(x_n - x) f'(x_n) - f(x_n) + f(x)}{f'(x_n)}
$$

  Pero como $f(x) = 0$, se tiene:

\begin{align*}
x_{n+1} - x &= x_n - \frac{f(x_n)}{f'(x_n)} - x \\
&= \frac{f'(x_n)x_n - f(x_n)}{f'(x_n)} - x \\
&= \frac{f'(x_n)x_n - f(x_n) - f'(x_n)x}{f'(x_n)} \\
&= \frac{f'(x_n)(x_n - x) - f(x_n)}{f'(x_n)} \\
&= \frac{(x_n - x) f'(x_n) - f(x_n)}{f'(x_n)} \quad \blacksquare
\end{align*}

c. Queremos probar que existen $\epsilon_1 > 0$ y $M > 0$ tales que:

$$
\forall y \in [x - \epsilon_1, x + \epsilon_1] \Rightarrow |f'(y)| \geq M
$$

  Dado que $f'(x) \neq 0$ y $f'$ es continua, existe un $\epsilon_1 > 0$ tal que:

$$
f'(y) \neq 0 \quad \forall y \in [x - \epsilon_1, x + \epsilon_1]
$$

  Esto implica que:

$$
M := \inf_{y \in [x - \epsilon_1, x + \epsilon_1]} |f'(y)| > 0 \quad \blacksquare
$$
d) Queremos justificar que:

$$
\max_{y \in [x - \epsilon_1, x + \epsilon_1]} |f''(y)| := L < \infty
$$

  Dado que $f''(y)$ es continua en $[x - \epsilon_1, x + \epsilon_1]$, un intervalo cerrado, alcanza un máximo en ese intervalo. Por lo tanto:

$$
L := \max_{y \in [x - \epsilon_1, x + \epsilon_1]} |f''(y)| < \infty \quad \blacksquare
$$
e) Queremos probar que el valor absoluto de la parte derecha del numerador de la ecuación (1) es igual a:

$$
\left| \int_{x_n}^x f''(t)(x - t) \, dt \right|
$$
...
\Rightarrow f(x) - f(x_n) - f'(x_n)(x - x_n) &= \int_{x_n}^x f'(t) \, dt - f'(x_n)(x - x_n) \\
&= \int_{x_n}^x \left(f'(t) - f'(x_n)\right) dt \tag{★}
\end{align*}

  Por otra parte, usando el teorema fundamental del cálculo:

\begin{align*}
f'(t) - f'(x_n) = \int_{x_n}^t f''(s) \, ds
\end{align*}

  Sustituyendo en (★):

\begin{align*}
\int_{x_n}^x (f'(t) - f'(x_n)) dt &= \int_{x_n}^x \left( \int_{x_n}^t f''(s) \, ds \right) dt
\end{align*}

  Intercambiamos el orden de integración (ver gráfica triangular en $s$-$t$):

\begin{align*}
\int_{x_n}^x \left( \int_{x_n}^t f''(s) \, ds \right) dt &= \int_{x_n}^x \left( \int_{s}^x dt \right) f''(s) \, ds \\
&= \int_{x_n}^x (x - s) f''(s) \, ds
\end{align*}

  Entonces:

\begin{align*}
f(x) - f(x_n) - f'(x_n)(x - x_n) &= \int_{x_n}^x (x - s) f''(s) \, ds
\end{align*}

  Haciendo cambio de variable $t = s$, se concluye:

$$
\left| f(x) - f(x_n) - f'(x_n)(x - x_n) \right| = \left| \int_{x_n}^x f''(t)(x - t) \, dt \right| \quad \blacksquare
$$

f) Probar que la expresión:
...
\begin{align*}
\left| \int_{x_n}^x f''(t)(x - t) \, dt \right|
&\leq \int_{x_n}^x |f''(t)| \cdot |x - t| \, dt \\
&\leq L \int_{x_n}^x |x - t| \, dt
\end{align*}

  Calculamos:

\begin{align*}
\int_{x_n}^x |x - t| \, dt &= \int_{x_n}^x (x - t) \, dt \\
&= \left[ x t - \frac{t^2}{2} \right]_{x_n}^x \\
&= \left( x^2 - \frac{x^2}{2} \right) - \left( x x_n - \frac{x_n^2}{2} \right) \\
&= \frac{x^2}{2} - x x_n + \frac{x_n^2}{2} \\
&= \frac{(x - x_n)^2}{2}
\end{align*}
...
$$
\left| \int_{x_n}^x f''(t)(x - t) \, dt \right| \leq \frac{L}{2} |x - x_n|^2 \quad \blacksquare
$$

g) Probar que si se define:
...
$$
x_{n+1} - x = \frac{\int_{x_n}^x f''(t)(x - t) \, dt}{f'(x_n)}
$$

  Aplicando el resultado del ítem **(f)**:

\begin{align*}
|x_{n+1} - x| &\leq \frac{1}{|f'(x_n)|} \left| \int_{x_n}^x f''(t)(x - t) \, dt \right| \\
&\leq \frac{L}{2M} |x_n - x|^2
\end{align*}

  Luego, si definimos $r_n = \dfrac{L}{2M} |x_n - x|$, entonces:

$$
|x_{n+1} - x| \leq r_n^2 \quad \blacksquare
$$

h) 
$$
\varepsilon < \min\left\{ \varepsilon_1, \frac{2M}{L} \right\}
$$

  Probar que si $|x_n - x| < \varepsilon$, entonces $|x_{n+1} - x| < \varepsilon$, y concluir que la sucesión converge.

  Sabemos que:

$$
|x_{n+1} - x| \leq r_n^2 = \left( \frac{L}{2M} |x_n - x| \right)^2
$$

  Si $|x_n - x| < \varepsilon$, se tiene:

\begin{align*}
|x_{n+1} - x| &\leq \left( \frac{L}{2M} \right)^2 \varepsilon^2 \\
&< \left( \frac{L}{2M} \cdot \varepsilon \right) \\
&< \varepsilon
\end{align*}

  Ya que $\varepsilon < \dfrac{2M}{L}$, entonces $\dfrac{L}{2M} \cdot \varepsilon < 1$, y por tanto $\left( \dfrac{L}{2M} \cdot \varepsilon \right)^2 < \varepsilon$.

  Por lo tanto:

$$
|x_{n+1} - x| < \varepsilon \quad \blacksquare
$$

  Esto demuestra que la sucesión generada por el método de Newton-Raphson converge.
...
a) Sea $(r_n)$ una sucesión de números reales positivos tales que

$$
r_{n+1} \le r_n r_{n-1}.
$$

  **Prueba.**

    1.  Si $r_0<1$ y $r_1<1$, entonces por inducción $r_n\le 1$ para todo $n$.\
    2.  Usando los números de Fibonacci $F_n$, se obtiene:

$$
r_n \le r_0^{F_{n-1}} r_1^{F_n}.
$$

  Como $F_n \sim \tfrac{1}{\sqrt{5}} \rho^n$ con $\rho=\tfrac{1+\sqrt{5}}{2}\approx 1.618$, resulta que

$$
r_n \le C \,\tilde r^{\rho^n}, \qquad 0<\tilde r<1.
$$

  En particular, $r_n\to 0$ por comparación.
...
  Como $x$ es raíz de f, $f(x)=0$ y:

$$
f[x_n,x] = \frac{f(x_n)-f(x)}{x_n-x} = \frac{f(x_n)}{x_n-x}.
$$
...
f(x_n)=(x_n-x)\,f[x_n,x].
$$
c) De la fórmula del método de la secante:

$$
x_{n+1}=x_n-\frac{f(x_n)}{f[x_n,x_{n-1}]},
$$

  restando $x$ y usando (b):

$$
x_{n+1}-x = (x_n-x)\,\frac{f[x_n,x_{n-1}]-f[x_n,x]}{f[x_n,x_{n-1}]}.
$$
d) Por definición del cociente dividido de orden 2:

$$
f[x_n,x_{n-1},x] = \frac{f[x_n,x_{n-1}] - f[x_{n-1},x]}{x_n-x}.
$$

  Entonces:

$$
f[x_n,x_{n-1}]-f[x_n,x] = (x-x_{n-1})\,f[x_n,x_{n-1},x].
$$

  Sustituyendo en (c):

$$
x_{n+1}-x=\frac{(x_n-x)(x-x_{n-1})\,f[x_n,x_{n-1},x]}{f[x_n,x_{n-1}]}.
$$
e) Sea $[x-\varepsilon_1,x+\varepsilon_1]$ un intervalo sobre el cual $|f'(t)|\ge M>0$ para todo $t$ en el intervalo. Supongamos que $x_n$ y $x_{n-1}$ están en $[x-\varepsilon_1,x+\varepsilon_1]$. Por el teorema del valor medio existe $\xi$ entre $x_n$ y $x_{n-1}$ tal que: $$ f[x_n,x\_{n-1}] = \frac{f(x_n)-f(x_{n-1})}{x_n-x_{n-1}} = f'(\xi)$$ Como $|f'(\xi)|\ge M$ obtenemos: $$|f[x_n,x_{n-1}]|\ge M $$

f) Sea $L$ la cota superior de $|f''|$ en $[x-\varepsilon_1,x+\varepsilon_1]$, es decir $|f''(t)|\le L$ para todo $t$ en ese intervalo. Supongamos nuevamente que $x_n$ y $x_{n-1}$ están en $[x-\varepsilon_1,x+\varepsilon_1]$. Usando la fórmula del cociente dividido de segundo orden y la representación con derivadas (o la forma simétrica de la expansión de Taylor), existe $\eta$ entre los tres puntos $x_n,x_{n-1},x$ tal que: $$ f[x_n,x_{n-1},x] = \frac{f''(\eta)}{2} $$

  Por tanto, tomando módulo: $$ |f[x_n,x_{n-1},x]|\le \frac{L}{2} $$ 
  De aquí, usando la definición de $f[x_n,x_{n-1},x]$ se obtiene la cota buscada: $$ |f[x_n,x_{n-1},x]|\le \frac{L}{2}. $$

g) Partiendo de la identidad de la parte (d) $$ x_{n+1}-x=\frac{(x_n-x)(x-x_{n-1})\,f[x_n,x_{n-1},x]}{f[x_n,x_{n-1}]} $$
  tomando módulos y aplicando las cotas $|f[x_n,x_{n-1},x]|\le L/2$ y $|f[x_n,x_{n-1}]|\ge M$ (obtenidas en e) y f)) obtenemos:
$$ |x_{n+1}-x| \le \frac{L/2}{M},|x_n-x|,|x-x\_{n-1}| = \frac{L}{2M},|x_n-x|,|x_{n-1}-x| $$

  Así se demuestra la desigualdad buscada: $$ |x_{n+1}-x| \le \frac{L}{2M},|x_n-x|,|x_{n-1}-x|$$

h) Defina la sucesión $$ r_n := \frac{L}{2M},|x_n-x|.$$ Fije además $$\varepsilon < \min\big(\varepsilon*1,;* \tfrac{2M}{L}\big)$$

  Si $|x_0-x|<\varepsilon$ y $|x_1-x|<\varepsilon$, entonces por la elección de $\varepsilon$ se tiene $r_0<1$ y $r_1<1$. De la desigualdad de la parte (g) se obtiene, multiplicando por $L/(2M)$:

$$ r{n+1} = \frac{L}{2M}|x_{n+1}-x| \le \frac{L}{2M}\cdot\frac{L}{2M}\cdot|x_n-x|\cdot |x_{n-1}-x| = r_n r\_{n-1} $$

  Por tanto la sucesión $(r_n)$ satisface la hipótesis de la parte (a):

$$ r\_{n+1}\le r_n r\_{n-1}$$

  Como $r_0<1$ y $r_1<1$, por la parte (a) concluimos que $(r_n)$ está acotada por $1$ y $r_n\to 0$ cuando $n\to\infty$. De la definición de $r_n$ esto implica

$$ |x_n-x| = \frac{2M}{L} r_n \longrightarrow 0$$

  es decir $x_n\to x$. Además, por la cota exponencial (parte a) existe $C>0$ y $0<\tilde r<1$ tales que

$$ r_n \le C\cdot\tilde r^{\rho^n};\qquad \rho=\frac{1+\sqrt5}{2}$$

  Despejando $|x_n-x|$ se obtiene una cota: $$ |x_n-x| \le \frac{2M}{L}\cdot C\cdot\tilde r^{\rho^n}$$

  lo que muestra que la convergencia es superlineal con orden $\rho\approx1.618$ (orden de la secante).
...
Como $f'(\alpha)\ne 0$ y $f'$ es continua, 
$$\exists\;\varepsilon>0\;:\;f'(x)\neq 0\;\forall\;x\in I:=[\alpha-\varepsilon,\alpha+\varepsilon]$$Luego $f$ es estrictamente monótona en $I$.  
...
$$
por lo que todas las iteraciones permanecen en $I$ y la sucesión $\{e_n\}$ es decreciente a $0$. Concluya que, para $x_0,x_1$ suficientemente cercanos a $\alpha$, la secuencia generada por la secante está bien definida en $I$ y **converge a $\alpha$**.
...
El método $\triangle^2$ de Aitken se puede escribir como:
