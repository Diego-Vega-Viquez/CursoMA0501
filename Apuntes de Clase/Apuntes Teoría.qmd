...
# Algebra lineal numérica – Solución numérica de sistemas de ecuaciones.

## Sistemas de Ecuaciones no Lineales

### Método de Punto Fijo

- Un sistema de ecuaciones **no lineal** tiene la forma:
...
donde cada $f_i : \mathbb{R}^n \to \mathbb{R}$.

- Este sistema puede ser transformado definiendo:
...
$$
F(x_1, x_2, \dots, x_n) = (f_1(x_1, \dots, x_n), f_2(x_1, \dots, x_n), \dots, f_n(x_1, \dots, x_n))^t.
$$

Entonces, el sistema anterior se escribe como:

$$
F(\mathbf{x}) = 0, \quad \text{con } \mathbf{x} = (x_1, x_2, \dots, x_n)^t.
$$

::: {.callout-note title="Definición: Límite de una función vectorial"}
Sea $F : D \subseteq \mathbb{R}^n \to \mathbb{R}^n$ de la forma

$$
F(\mathbf{x}) = (f_1(\mathbf{x}), f_2(\mathbf{x}), \dots, f_n(\mathbf{x}))^t,
$$

y sean $\mathbf{x}_0 \in D$ y $\mathbf{L} = (L_1, L_2, \dots, L_n)^t$.  
Se define:

$$
\lim_{\mathbf{x} \to \mathbf{x}_0} F(\mathbf{x}) = \mathbf{L}
\iff
\lim_{\mathbf{x} \to \mathbf{x}_0} f_i(\mathbf{x}) = L_i, \quad i = 1, 2, \dots, n,
$$
...
Sea $F : D \subseteq \mathbb{R}^n \to \mathbb{R}^n$.  
Decimos que $F$ es continua en $\mathbf{x}_0 \in D$ si existe $\lim_{\mathbf{x} \to \mathbf{x}_0} F(\mathbf{x})$ y

$$
\lim_{\mathbf{x} \to \mathbf{x}_0} F(\mathbf{x}) = F(\mathbf{x}_0).
$$

Además, $F$ es continua en $D$ si es continua en todo $\mathbf{x} \in D$.
:::

::: {.callout-note title="Teorema"}
Sea $f : D \subseteq \mathbb{R}^n \to \mathbb{R}$ y $\mathbf{x}_0 \in D$.  
Si existen constantes $\delta > 0$ y $K > 0$ tales que:

$$
\left| \frac{\partial f(\mathbf{x})}{\partial x_j} \right| \le K
\quad \text{para todo } j = 1, 2, \dots, n,
$$

siempre que $\|\mathbf{x} - \mathbf{x}_0\| < \delta$ y $\mathbf{x} \in D$,  
entonces $f$ es continua en $\mathbf{x}_0$.

::: {.callout-caution collapse="true" title="Demostración"}
Sea $f : D \subseteq \mathbb{R}^n \to \mathbb{R}$ y $\mathbf{x}_0 \in D$.
Supongamos que existen constantes $\delta > 0$ y $K > 0$ tales que
...
Queremos probar que $f$ es continua en $\mathbf{x}0$, es decir, que
$$
\lim{\mathbf{x} \to \mathbf{x}_0} f(\mathbf{x}) = f(\mathbf{x}_0).
$$

Si $f$ es diferenciable en una vecindad de $\mathbf{x}_0$, entonces para todo
$\mathbf{x} \in D$ tal que $|\mathbf{x} - \mathbf{x}_0| < \delta$, existe un punto
$\mathbf{c}$ en el segmento que une $\mathbf{x}_0$ con $\mathbf{x}$ tal que

$$
f(\mathbf{x}) - f(\mathbf{x}_0) = \nabla f(\mathbf{c}) \cdot (\mathbf{x} - \mathbf{x}_0).
$$

Por definición de producto punto, esto implica que

$$
|f(\mathbf{x}) - f(\mathbf{x}0)| \le |\nabla f(\mathbf{c})|\infty , |\mathbf{x} - \mathbf{x}_0|,
$$

donde $|\nabla f(\mathbf{c})|_\infty = \max_j \left| \frac{\partial f(\mathbf{c})}{\partial x_j} \right|$.

De la hipótesis se tiene que para todo $\mathbf{c}$ suficientemente cercano a $\mathbf{x}_0$,

$$
\left| \frac{\partial f(\mathbf{c})}{\partial x_j} \right| \le K,
\quad \forall j = 1, 2, \dots, n,
$$

por lo tanto, $|\nabla f(\mathbf{c})|_\infty \le K$.

Sustituyendo en la desigualdad anterior, obtenemos

$$
|f(\mathbf{x}) - f(\mathbf{x}_0)| \le K , |\mathbf{x} - \mathbf{x}_0|.
$$

Dado que $K$ es una constante, para toda $\varepsilon > 0$ basta elegir
$\delta = \varepsilon / K$ (o cualquier número positivo menor) para asegurar que,
si $|\mathbf{x} - \mathbf{x}_0| < \delta$, entonces

$$
|f(\mathbf{x}) - f(\mathbf{x}_0)| < \varepsilon.
$$

Esto prueba que $f$ es continua en $\mathbf{x}_0$. $\blacksquare$
...
::: {.callout-note title="Teorema"}
Sea $D \subset \mathbb{R}^n$ un conjunto abierto y convexo, y sea $F : D \to \mathbb{R}^n$ la función:

$$
F(\mathbf{x}) = (f_1(x_1, x_2, \dots, x_n), f_2(x_1, x_2, \dots, x_n), \dots, f_n(x_1, x_2, \dots, x_n))^t,
$$

donde cada $f_j : D \to \mathbb{R}$, $j = 1, 2, \dots, n$, es continuamente diferenciable.  

Se denota:

$$
F'(\mathbf{x}) = 
\left( \frac{\partial f_j}{\partial x_k}(\mathbf{x}) \right)_{j,k=1,2,\dots,n}
$$

como la matriz **Jacobiana** de $F$.  
Entonces se cumple el **Teorema del Valor Medio**:

$$
\| F(\mathbf{x}) - F(\mathbf{y}) \|
\le
\max_{0 \le \lambda \le 1} \| F'[\lambda \mathbf{x} + (1 - \lambda)\mathbf{y}] \| \, \|\mathbf{x} - \mathbf{y}\|,
$$

para todo $\mathbf{x}, \mathbf{y} \in D$.

::: {.callout-caution collapse="true" title="Demostración"}
Sea $D \subset \mathbb{R}^n$ abierto y convexo, y $F : D \to \mathbb{R}^n$ de la forma

$$
F(\mathbf{x}) = (f_1(\mathbf{x}), f_2(\mathbf{x}), \dots, f_n(\mathbf{x}))^t,
$$

donde cada $f_j$ es continuamente diferenciable en $D$.

Tomemos $\mathbf{x}, \mathbf{y} \in D$.
Como $D$ es convexo, para todo $\lambda \in [0,1]$ se tiene
$$
\mathbf{z}(\lambda) := \lambda \mathbf{x} + (1 - \lambda)\mathbf{y} \in D.
$$

Definimos la función auxiliar $\Phi : [0,1] \to \mathbb{R}^n$ dada por
$$
\Phi(\lambda) := F(\mathbf{z}(\lambda)) = F(\lambda \mathbf{x} + (1-\lambda)\mathbf{y}).
$$

Entonces:
	•	$\Phi$ es continuamente diferenciable en $[0,1]$ porque $F$ lo es en $D$ y $\mathbf{z}(\lambda)$ es diferenciable.
	•	Notemos que
$$
\Phi(1) = F(\mathbf{x}), \qquad \Phi(0) = F(\mathbf{y}).
$$

Por lo tanto:
$$
F(\mathbf{x}) - F(\mathbf{y}) = \Phi(1) - \Phi(0) = \int_0^1 \Phi’(\lambda), d\lambda.
$$

Ahora calculemos $\Phi’(\lambda)$ usando la regla de la cadena.
Para $\mathbf{z}(\lambda) = \lambda \mathbf{x} + (1-\lambda)\mathbf{y}$ se tiene
$$
\frac{d\mathbf{z}}{d\lambda} = \mathbf{x} - \mathbf{y}.
$$

Luego, por la regla de la cadena en dimensión múltiple:
$$
\Phi’(\lambda)
= F’(\mathbf{z}(\lambda)) , \frac{d\mathbf{z}}{d\lambda}
= F’(\mathbf{z}(\lambda))(\mathbf{x} - \mathbf{y}),
$$
donde $F’(\mathbf{z}(\lambda))$ es la matriz Jacobiana de $F$ evaluada en $\mathbf{z}(\lambda)$.

Así,
$$
F(\mathbf{x}) - F(\mathbf{y})
= \int_0^1 F’(\mathbf{z}(\lambda))(\mathbf{x} - \mathbf{y}), d\lambda.
$$

Tomando norma en ambos lados y usando la desigualdad triangular:
$$
|F(\mathbf{x}) - F(\mathbf{y})|
\le \int_0^1 | F’(\mathbf{z}(\lambda))(\mathbf{x} - \mathbf{y}) | , d\lambda.
$$

Usando submultiplicatividad de la norma matricial (i.e. $|A\mathbf{v}| \le |A|,|\mathbf{v}|$):
$$
|F(\mathbf{x}) - F(\mathbf{y})|
\le \int_0^1 |F’(\mathbf{z}(\lambda))| ,|\mathbf{x} - \mathbf{y}| , d\lambda
= |\mathbf{x} - \mathbf{y}| \int_0^1 |F’(\mathbf{z}(\lambda))| , d\lambda.
$$

Como $\lambda \mapsto |F’(\mathbf{z}(\lambda))|$ es continua en $[0,1]$, está acotada. Por lo tanto,
$$
\int_0^1 |F’(\mathbf{z}(\lambda))| , d\lambda
\le \max_{0 \le \lambda \le 1} |F’(\mathbf{z}(\lambda))|.
$$
...
|F(\mathbf{x}) - F(\mathbf{y})|
\le
\left( \max_{0 \le \lambda \le 1} |F’(\lambda \mathbf{x} + (1-\lambda)\mathbf{y})| \right)
|\mathbf{x} - \mathbf{y}|.
$$
...
\max_{0 \le \lambda \le 1} | F’[\lambda \mathbf{x} + (1 - \lambda)\mathbf{y}] | , |\mathbf{x} - \mathbf{y}|,
$$
para todo $\mathbf{x}, \mathbf{y} \in D$, que es exactamente la afirmación del teorema. $\blacksquare$
...
::: {.callout-note title="Teorema"}
Sea $D \subset \mathbb{R}^n$ un conjunto cerrado y convexo (con $\overset{\circ}{D} \ne \emptyset$), y sea  
$F : D \to D$ una función continua. Además, se asume que $F$ es continuamente diferenciable en el interior de $D$, y que su jacobiano puede ser continuamente extendido a todo $D$, de modo que:

$$
\sup_{\mathbf{x} \in D} \|F'(\mathbf{x})\| < 1,
$$

donde $\|\cdot\|$ denota alguna norma en $\mathbb{R}^n$.  
Entonces:

- La ecuación $F(\mathbf{x}) = \mathbf{x}$ tiene una solución única en $D$.

- Las aproximaciones sucesivas

$$
\mathbf{x}_{n+1} = F(\mathbf{x}_n), \quad n = 0, 1, 2, \dots,
$$

con $\mathbf{x}_0 \in D$, convergen a este punto fijo.

- Se cumple el error *a priori*:

$$
\|\mathbf{x}_n - \mathbf{x}\| \le \frac{q^n}{1 - q} \|\mathbf{x}_1 - \mathbf{x}_0\|, 
\quad \forall n \in \mathbb{N},
$$

donde $q = \sup_{\mathbf{x} \in D} \|F'(\mathbf{x})\|$.

- Se tiene el error *a posteriori*:

$$
\|\mathbf{x}_n - \mathbf{x}\| \le \frac{q}{1 - q} \|\mathbf{x}_n - \mathbf{x}_{n-1}\|, 
\quad \forall n \in \mathbb{N}.
$$

::: {.callout-caution collapse="true" title="Prueba"}
Por el teorema anterior, $F : D \to D$, y usando que  
$\displaystyle \sup_{\mathbf{x} \in D} \|F'(\mathbf{x})\| < 1$,  
se tiene que $F$ es una **contracción**.  
Luego, aplicando el **Teorema del Punto Fijo de Banach**, se obtiene el resultado. $\blacksquare$
:::

::: 

::: {.callout-important collapse="true" title="Observación"}
Cualquiera de las siguientes condiciones garantiza la convergencia de las aproximaciones sucesivas:

$$
\sup_{\mathbf{x} \in D} 
\max_{j = 1, 2, \dots, n}
\sum_{k=1}^n \left| \frac{\partial f_j(\mathbf{x})}{\partial x_k} \right| < 1,
$$

$$
\sup_{\mathbf{x} \in D} 
\max_{k = 1, 2, \dots, n}
\sum_{j=1}^n \left| \frac{\partial f_j(\mathbf{x})}{\partial x_k} \right| < 1,
$$
...
\sum_{j,k=1}^n
\left(
\frac{\partial f_j(\mathbf{x})}{\partial x_k}
\right)^2
\right]^{1/2}
< 1.
$$
...
::: {.callout-note title="Teorema"}
Sea $\mathbf{x}$ un punto fijo de una función continuamente diferenciable $F$ tal que
$|F’(\mathbf{x})| < 1$ para alguna norma $|\cdot|$ en $\mathbb{R}^n$.
Entonces, el método de aproximaciones sucesivas:

$$
\mathbf{x}_{n+1} = F(\mathbf{x}_n)
$$

es localmente convergente.

Es decir, existe un vecindario $B$ del punto fijo $\mathbf{x}$ tal que las aproximaciones sucesivas convergen a $\mathbf{x}$ para todo $\mathbf{x}_0 \in B$.

::: {.callout-caution collapse="true" title="Demostración"}
Sea $\mathbf{x}^{*}$ un punto fijo de $F$, es decir, $F(\mathbf{x}^{*}) = \mathbf{x}^{*}$.
Dado que $F$ es continuamente diferenciable en un entorno de $\mathbf{x}^{*}$, podemos aplicar el Teorema del Valor Medio en varias variables.
Así, para todo $\mathbf{x}$ cercano a $\mathbf{x}^{*}$, existe un punto $\boldsymbol{\xi}$ en el segmento que une $\mathbf{x}$ y $\mathbf{x}^{*}$ tal que:

$$
F(\mathbf{x}) - F(\mathbf{x}^{*}) = F’(\boldsymbol{\xi})(\mathbf{x} - \mathbf{x}^{*}).
$$

Tomando norma en ambos lados:

$$
|F(\mathbf{x}) - F(\mathbf{x}^{*})| \le |F^\prime(\boldsymbol{\xi})| , |\mathbf{x} - \mathbf{x}^{*}|.
$$

Por hipótesis, $|F’(\mathbf{x}^{*})| < 1$.
Como $F’$ es continua, existe un entorno $B$ de $\mathbf{x}^{*}$ tal que

$$
|F^\prime(\mathbf{x})| \le q < 1, \quad \forall \mathbf{x} \in B.
$$

Entonces, para todo $\mathbf{x} \in B$:

$$
|F(\mathbf{x}) - F(\mathbf{x}^{*})| \le q , |\mathbf{x} - \mathbf{x}^{*}|.
$$

Esto muestra que $F$ es una contracción local en $B$.

Por el Teorema del Punto Fijo de Banach, toda función contracción tiene un único punto fijo en $B$, y las iteraciones

$$
\mathbf{x}_{n+1} = F(\mathbf{x}_n)
$$

convergen a $\mathbf{x}^{*}$ para todo $\mathbf{x}_0 \in B$.

Por lo tanto, el método de aproximaciones sucesivas es localmente convergente. $\blacksquare$
:::

:::

::: {.callout-note title="Definición: Punto fijo"}
Una función $G : D \subseteq \mathbb{R}^n \to \mathbb{R}^n$ tiene un **punto fijo** en $\mathbf{p} \in D$ si  
...
::: {.callout-note title="Teorema: Funciones contractivas"}
Sea  

$$
D = \{(x_1, x_2, \dots, x_n)^t \mid a_i \le x_i \le b_i, \, i = 1, 2, \dots, n\},
$$

donde los $a_i$ y $b_i$ son constantes.  
Además, sea $G : D \subseteq \mathbb{R}^n \to \mathbb{R}^n$ una función continua tal que $G(\mathbf{x}) \in D$ para todo $\mathbf{x} \in D$.  

**Entonces**, $G$ tiene un punto fijo en $D$.

Si además $G$ tiene derivadas parciales continuas y existe una constante $K < 1$ tal que:

$$
\left| \frac{\partial g_i(\mathbf{x})}{\partial x_j} \right| \le \frac{K}{n}, 
\quad \forall \mathbf{x} \in D, \, j = 1, 2, \dots, n,
$$

entonces la sucesión $\{\mathbf{x}_k\}_{k \in \mathbb{N}}$ definida por  

$$
\mathbf{x}_k = G(\mathbf{x}_{k-1}), 
\quad k \ge 1, \, \mathbf{x}_0 \in D,
$$

converge al punto fijo único de $G$, denotado por $\mathbf{p} \in D$.

Además, se cumple el **error a priori**:

$$
\|\mathbf{x}_k - \mathbf{p}\|_\infty \le \frac{K^k}{1 - K} \, \|\mathbf{x}_1 - \mathbf{x}_0\|_\infty.
$$

::: {.callout-caution collapse="true" title="Demostración"}
**1. Existencia del punto fijo**

Como $G$ es continua y transforma el conjunto cerrado y convexo $D$ en sí mismo, por el **Teorema del Punto Fijo de Brouwer**, existe al menos un punto $\mathbf{p} \in D$ tal que  

$$
G(\mathbf{p}) = \mathbf{p}.
$$

**2. Unicidad y convergencia**

De la hipótesis, las derivadas parciales satisfacen  

$$
\left| \frac{\partial g_i(\mathbf{x})}{\partial x_j} \right| \le \frac{K}{n}, 
\quad \text{para todo } \mathbf{x} \in D,
$$

lo que implica que la norma del Jacobiano $\|G'(\mathbf{x})\|_\infty \le K < 1$.  
Por tanto, $G$ es una **contracción** en $D$ con constante de contracción $K$:

$$
\|G(\mathbf{x}) - G(\mathbf{y})\|_\infty \le K \|\mathbf{x} - \mathbf{y}\|_\infty, 
\quad \forall \mathbf{x}, \mathbf{y} \in D.
$$

Aplicando el **Teorema del Punto Fijo de Banach**, se concluye que:

- $G$ tiene un único punto fijo $\mathbf{p}$ en $D$,
- y la sucesión $\mathbf{x}_{k+1} = G(\mathbf{x}_k)$ converge a $\mathbf{p}$ para cualquier $\mathbf{x}_0 \in D$.

**3. Cota de error**

Del carácter contractivo de $G$, se tiene:

$$
\|\mathbf{x}_k - \mathbf{p}\|_\infty 
\le K \|\mathbf{x}_{k-1} - \mathbf{p}\|_\infty 
\le K^k \|\mathbf{x}_0 - \mathbf{p}\|_\infty.
$$

Sumando las desigualdades de manera telescópica y usando que  

$$
\|\mathbf{x}_1 - \mathbf{p}\|_\infty 
\le K \|\mathbf{x}_0 - \mathbf{p}\|_\infty,
$$

se obtiene la cota de error a priori:

$$
\|\mathbf{x}_k - \mathbf{p}\|_\infty 
\le \frac{K^k}{1 - K} \|\mathbf{x}_1 - \mathbf{x}_0\|_\infty.
$$

De este modo, la sucesión converge geométricamente hacia el punto fijo único $\mathbf{p}$. $\blacksquare$

:::

:::

::: {.callout-note title="Algoritmo: Método de Iteración de Punto Fijo para funciones vectoriales"}

**Datos:**
- Función $G : D \subseteq \mathbb{R}^n \to \mathbb{R}^n$  
- Punto inicial $\mathbf{x}_0 \in D$  
- Tolerancia $\varepsilon > 0$  
- Número máximo de iteraciones $N_{\max}$  

**Resultado:**  
Aproximación al punto fijo $\mathbf{p}$ tal que $G(\mathbf{p}) = \mathbf{p}$.

**Procedimiento:**

Para $k = 1, 2, \dots, N_{\max}$ hacer:
...
2. Si  
   $$
   \|\mathbf{x}_k - \mathbf{x}_{k-1}\|_\infty < \varepsilon,
   $$  
   entonces detener y asignar $\mathbf{p} = \mathbf{x}_k$.

Si no se cumple la condición en $N_{\max}$ iteraciones, imprimir:  
**"No converge en $N_{\max}$ iteraciones."**

::: {.callout-caution collapse="true" title="Algoritmo R"}

#### Punto Fijo en Varias Variables

```{r}
#| code-fold: true

# Iteración de Punto Fijo para funciones vectoriales
# P0      : vector inicial c(x1, x2, ..., xn)
# tol     : tolerancia
# n.max   : número máximo de iteraciones
# G       : lista de n funciones (g1, g2, ..., gn)
#           Cada g_j puede ser g_j(x) (vector) o g_j(x1, ..., xn) (escalares).
# desplegar : imprimir o no el progreso

punto.fijo.vv <- function(P0, tol = 1e-8, n.max = 100, G, desplegar = TRUE) {
  x.prev <- as.numeric(P0)
  m <- length(x.prev)
  if (length(G) != m) 
    stop("G debe tener la misma longitud que P0.")
  x.next <- x.prev
  # Evalúa la j-ésima componente de G en x 
  evaluar.componente <- function(gj, x) {
    # 1) gj(x) (vector)
    v <- try(gj(x), silent = TRUE)
    if (!inherits(v, "try-error") && length(v) == 1 && is.finite(v)) 
      return(as.numeric(v))
    # 2) gj(x1, ..., xn)
    as.numeric(do.call(gj, as.list(x)))
  }
  for (i in seq_len(n.max)) {
    for (j in seq_len(m)) 
      x.next[j] <- evaluar.componente(G[[j]], x.prev)
    if (desplegar) {
      cat(sprintf("Iteración %d: P = (%s)\n",
                  i, paste(formatC(x.next, digits = 15, format = "g"), collapse = ", ")))
    }
    if (max(abs(x.next - x.prev)) < tol) 
      return(x.next)
    x.prev <- x.next
  }
  message("El método no converge")
  return(x.next)
}
```
...
g1 <- function(x1, x2, x3) (1/3)*cos(x2*x3) + 1/6
g2 <- function(x1, x2, x3) (1/9)*sqrt(x1^2 + sin(x3) + 1.06) - 0.1
g3 <- function(x1, x2, x3) - (1/20)*exp(-x1*x2) - (10*pi - 3)/60

G <- list(g1, g2, g3)

p0    <- c(0, 0, 0)
sol   <- punto.fijo.vv(p0, tol = 1e-8, n.max = 100, G = G, desplegar = TRUE)
...
##### Ejemplo: $x^2−10x+y^2+8=0$,$xy^2+x−10y+8=0$. Solución $(1,1)$

```{r}
#| code-fold: true

g1 <- function(x1, x2) 0.1 * (x1^2 + x2^2 + 8)
g2 <- function(x1, x2) 0.1 * (x1 * x2^2 + x1 + 8)

G  <- list(g1, g2)
P0 <- c(0, 0)

punto.fijo.vv(P0, tol = 1e-10, n.max = 100, G = G, desplegar = TRUE)
...
::: {.callout-tip collapse="true" title="Ejemplo"}
Resuelva usando iteración de punto fijo el sistema:

$$
\begin{cases}
3x_1 - \cos(x_2 x_3) - \dfrac{1}{2} = 0, \\
x_1^2 - 81(x_2 + 0.1)^2 + \sin x_3 + 1.06 = 0, \\
e^{-x_1 x_2} + 20x_3 + \dfrac{10\pi - 3}{3} = 0.
\end{cases}
$$

**Solución:**  
Este sistema se puede transformar en un problema de punto fijo como sigue:

$$
\begin{cases}
x_1 = \dfrac{1}{3}\cos(x_2 x_3) + \dfrac{1}{6} = g_1(x_1, x_2, x_3), \\
x_2 = \dfrac{1}{9}\sqrt{x_1^2 + \sin x_3 + 1.06} - 0.1 = g_2(x_1, x_2, x_3), \\
x_3 = -\dfrac{1}{20}e^{-x_1 x_2} - \dfrac{10\pi - 3}{60} = g_3(x_1, x_2, x_3).
\end{cases}
$$
...
$$
G : \mathbb{R}^3 \to \mathbb{R}^3, \quad \mathbf{x} = (x_1, x_2, x_3)^t,
$$

tal que  

$$
G(\mathbf{x}) = (g_1(\mathbf{x}), g_2(\mathbf{x}), g_3(\mathbf{x})) = (x_1, x_2, x_3) = \mathbf{x}.
$$

Demostraremos que $G(\mathbf{x})$ tiene un punto fijo único en:

$$
D = \{(x_1, x_2, x_3)^t \mid -1 \le x_i \le 1, \; i = 1, 2, 3\}.
$$

Veamos:

$$
|g_1(x_1, x_2, x_3)| = \left|\dfrac{1}{3}\cos(x_2 x_3) + \dfrac{1}{6}\right|
\le \dfrac{1}{3} + \dfrac{1}{6} \le \dfrac{1}{2},
$$

$$
|g_2(x_1, x_2, x_3)| = \left|\dfrac{1}{9}\sqrt{x_1^2 + \sin x_3 + 1.06} - 0.1\right|
\le \dfrac{1}{9}\sqrt{1 + 1 + 1.06} - 0.1 < 0.09,
$$

$$
|g_3(x_1, x_2, x_3)| = \left| -\dfrac{1}{20}e^{-x_1 x_2} - \dfrac{10\pi - 3}{60} \right|
\le \dfrac{1}{20}e + \dfrac{10\pi - 3}{60} < 0.61.
$$

Por lo tanto, $G(\mathbf{x}) \in D$ para todo $\mathbf{x} \in D$.

Las derivadas son acotadas en $D$:

**Acotación de la Jacobiana y contracción en** $D = [-1, 1]^3$

Recordemos:

$$
g_1(\mathbf{x}) = \tfrac{1}{3}\cos(x_2 x_3) + \tfrac{1}{6}, \quad
g_2(\mathbf{x}) = \tfrac{1}{9}\sqrt{x_1^2 + \sin x_3 + 1.06} - 0.1, \quad
g_3(\mathbf{x}) = -\tfrac{1}{20} e^{-x_1 x_2} - \tfrac{10\pi - 3}{60},
$$

con $\mathbf{x} = (x_1, x_2, x_3)^t$, $D = [-1, 1]^3$.

Calculamos la **Jacobiana** $G'(\mathbf{x})$ y sus cotas en $D$.

Usaremos que $|\sin(u)| \le \sin(1)$ para $|u| \le 1$ y $e^{|u|} \le e$ para $|u| \le 1$.

| Parcial | Expresión | Cota en $D$ |
|:--:|:--|:--:|
| $\dfrac{\partial g_1}{\partial x_1}$ | $0$ | $0$ |
| $\dfrac{\partial g_1}{\partial x_2}$ | $-\tfrac{1}{3}\sin(x_2 x_3)x_3$ | $\le \tfrac{1}{3}\sin(1)$ $(<0.281)$ |
| $\dfrac{\partial g_1}{\partial x_3}$ | $-\tfrac{1}{3}\sin(x_2 x_3)x_2$ | $\le \tfrac{1}{3}\sin(1)$ $(<0.281)$ |
| $\dfrac{\partial g_2}{\partial x_1}$ | $\tfrac{1}{9} \dfrac{x_1}{\sqrt{x_1^2 + \sin x_3 + 1.06}}$ | $\le \tfrac{1}{9\sqrt{1.06}}$ $(<0.108)$ |
| $\dfrac{\partial g_2}{\partial x_2}$ | $0$ | $0$ |
| $\dfrac{\partial g_2}{\partial x_3}$ | $\tfrac{1}{18}\dfrac{\cos x_3}{\sqrt{x_1^2 + \sin x_3 + 1.06}}$ | $\le \tfrac{1}{18\sqrt{1.06}}$ $(<0.054)$ |
| $\dfrac{\partial g_3}{\partial x_1}$ | $\tfrac{1}{20}x_2 e^{-x_1 x_2}$ | $\le \tfrac{e}{20}$ $(<0.136)$ |
| $\dfrac{\partial g_3}{\partial x_2}$ | $\tfrac{1}{20}x_1 e^{-x_1 x_2}$ | $\le \tfrac{e}{20}$ $(<0.136)$ |
| $\dfrac{\partial g_3}{\partial x_3}$ | $0$ | $0$ |

Para la norma infinito:

$$
\| G'(\mathbf{x}) \|_\infty = \max_i \sum_{j=1}^3 \left| \frac{\partial g_i}{\partial x_j}(\mathbf{x}) \right|.
$$

Con las cotas anteriores:


- Fila 1: $\le \tfrac{1}{3}\sin(1) + \tfrac{1}{3}\sin(1) < 0.562,$  
- Fila 2: $\le \tfrac{1}{9\sqrt{1.06}} + \tfrac{1}{18\sqrt{1.06}} < 0.162,$  
- Fila 3: $\le \tfrac{e}{20} + \tfrac{e}{20} < 0.272.$  

Por lo tanto:

$$
K_\infty := \sup_{\mathbf{x} \in D} \|G'(\mathbf{x})\|_\infty \le 0.562 < 1.
$$

**En Conclusión**

Con $K := \sup_{\mathbf{x} \in D} \|G'(\mathbf{x})\| < 1$ (por ejemplo, con la norma $\infty$),  
la aplicación $G : D \to D$ es una **contracción** en $D$.  

En consecuencia, por el **Teorema del Punto Fijo de Banach**:

- Existe un **único punto fijo** $\mathbf{p} \in D$ tal que $G(\mathbf{p}) = \mathbf{p}$.
- La **iteración de punto fijo** $\mathbf{x}_{k+1} = G(\mathbf{x}_k)$, con cualquier $\mathbf{x}_0 \in D$, **converge a** $\mathbf{p}$.
- Se verifican los **errores a priori** y **a posteriori**:

$$
\|\mathbf{x}_n - \mathbf{p}\| \le \frac{K^n}{1 - K} \|\mathbf{x}_1 - \mathbf{x}_0\|,
\qquad
\|\mathbf{x}_n - \mathbf{p}\| \le \frac{K}{1 - K} \|\mathbf{x}_n - \mathbf{x}_{n-1}\|.
$$

- Usando la **cota teórica del error**, se tiene:

$$
\|\mathbf{x}_5 - \mathbf{p}\|_\infty 
\le \frac{(0.843)^5(0.423)}{1 - 0.843} < 1.15.
$$

- El punto fijo es (según el programa en R):

$$
\mathbf{p} = (0.5,\, 0,\, \tfrac{\pi}{6})^t.
$$

- Por lo tanto, el **error real en** $\mathbf{x}_5$ es:

$$
\|\mathbf{x}_5 - \mathbf{p}\|_\infty \le 2 \times 10^{-8}.
$$
...
::: {.callout-note title="Definición: Método de Newton para sistemas no lineales"}

Sea $D \subset \mathbb{R}^n$ abierto y sea $F : D \to \mathbb{R}^n$ una función continuamente diferenciable tal que su matriz Jacobiana $F'(\mathbf{x})$ es no singular para todo $\mathbf{x} \in D$.

Entonces, el **método de Newton** para la solución del sistema de ecuaciones

$$
F(\mathbf{x}) = 0,
$$

está dado por el siguiente **esquema iterativo**:
...
con $\mathbf{x}_0 \in D$.
:::

::: {.callout-note title="Teorema: Convergencia del método de Newton"}

Sea $D \subset \mathbb{R}^n$ abierto y convexo, y sea $F : D \to \mathbb{R}^n$ continuamente diferenciable.  Sea $\|\cdot\|$ alguna norma en $\mathbb{R}^n$ y $\mathbf{x}_0 \in D$. Supóngase que se cumplen las siguientes condiciones:
...
 para alguna constante $\gamma > 0$.

**(b)** La matriz Jacobiana $F'(\mathbf{x})$ es no singular para todo $\mathbf{x} \in D$,  y existe una constante $\beta > 0$ tal que:

$$
\|[F'(\mathbf{x})]^{-1}\| \le \beta, \qquad \mathbf{x} \in D.
$$

**(c)** Para las constantes:

$$
\alpha := \|[F'(\mathbf{x}_0)]^{-1}F(\mathbf{x}_0)\|, 
...
**(d)** Sea $r := 2\alpha$.  Entonces la bola cerrada:

$$
B[\mathbf{x}_0, r] := \{\mathbf{x} : \|\mathbf{x} - \mathbf{x}_0\| \le r\} \subset D.
$$


**Entonces:**

1. El **método de Newton** es convergente y $F$ tiene un **cero único** $\mathbf{x}^* \in B[\mathbf{x}_0, r]$.

2. Si $\mathbf{x}_0 \in D$, la sucesión
...
  está bien definida y converge al cero de $F$, es decir:

$$
\lim_{n \to \infty} \mathbf{x}_n = \mathbf{x}^*.
$$

3. Se tiene el siguiente **estimado del error**:

$$
\|\mathbf{x}_n - \mathbf{x}^*\| \le 2\alpha q^{2^n - 1}, 
\qquad n = 0, 1, 2, \dots
$$

4. El método de Newton **converge cuadráticamente**.

:::

::: {.callout-important collapse="true" title="Observación"}
Este es uno de los teoremas más generales de **convergencia del método de Newton**.  
El problema práctico es que resulta muy poco aplicable debido a la dificultad para calcular los valores de $\alpha$, $\beta$ y $\gamma$.
:::

En la práctica, es más usado el siguiente teorema.

::: {.callout-note title="Teorema: Convergencia cuadrática del método de Newton"}

Sea $\mathbf{x}^*$ la solución de la ecuación:

$$
F(\mathbf{x}) = \mathbf{x},
$$

para $F = (f_1, f_2, \dots, f_n)^t$ una función de $\mathbb{R}^n$ en $\mathbb{R}^n$.  
Si existe un número $\delta > 0$ con las siguientes propiedades:

(a) $\dfrac{\partial f_i}{\partial x_j}$ es continua en:

$$
N_\delta := \{\mathbf{x} : \|\mathbf{x} - \mathbf{x}^*\| < \delta\}, 
\qquad i, j = 1, 2, \dots, n.
$$

(b) $\dfrac{\partial^2 f_i}{\partial x_j \partial x_k}$ es continua y satisface:

$$
\left| \frac{\partial^2 f_i}{\partial x_j \partial x_k}(\mathbf{x}) \right| \le M,
$$

  para alguna constante $M > 0$ y $i, j, k = 1, 2, \dots, n$.

(c) $\dfrac{\partial f_i}{\partial x_k}(\mathbf{x}^*) = 0$,  para $i, j, k = 1, 2, \dots, n$.

**Entonces:**

1. Existe $\hat{\delta} \le \delta$ tal que la **sucesión de Newton** converge cuadráticamente a $\mathbf{x}^*$, siempre que:

$$
\|\mathbf{x}_0 - \mathbf{x}^*\| < \hat{\delta}.
$$

2. El **error satisface**:

$$
\|\mathbf{x}_k - \mathbf{x}^*\|_\infty 
\le 
\frac{n^2 M}{2} 
\|\mathbf{x}_{k-1} - \mathbf{x}^*\|_\infty^2,
\qquad k \ge 1.
$$

:::

::: {.callout-note title="Algoritmo: Método de Newton para sistemas no lineales"}

**Entrada:**  
- Un sistema de $n$ ecuaciones $F(\mathbf{x}) = (f_1, \dots, f_n)^t$.  
- Aproximación inicial $\mathbf{x} = (x_1, \dots, x_n)^t$.  
- Tolerancia `tol`.  
- Número máximo de iteraciones $N$.

**Salida:**  
- Solución aproximada $\mathbf{x}$ o mensaje de error.

**Algoritmo:**

1. $k \leftarrow 1$  
2. **Mientras** $k \le N$ **hacer**  
   1. Calcular $F(\mathbf{x})$ y $F'(\mathbf{x})$ (Jacobiano).  
   ...
   3. Actualizar  
      ...
      $$  
   4. **Si** $\|\mathbf{y}\|_\infty < \text{tol}$ **entonces**  
      - **Salida:** $\mathbf{x}$  
      - **Parar**  
   5. $k \leftarrow k + 1$  
3. **Fin mientras**  
4. **Salida:** “Número máximo de iteraciones excedido”.
...
# ---------------------------------------------
# Utilidades: evaluar componente y G(x)
# Admite g_j(x) (vector) o g_j(x1, ..., xn) (escalares)
# ---------------------------------------------
evaluar.componente <- function(gj, x) {
  v <- try(gj(x), silent = TRUE)    # firma vectorial
  if (!inherits(v, "try-error") && length(v) == 1 && is.finite(v)) return(as.numeric(v))
  as.numeric(do.call(gj, as.list(x)))  # firma escalar
}

evaluar.G <- function(G, x) {
  vapply(G, evaluar.componente, numeric(1), x = x)
}

# ---------------------------------------------
# Jacobiana numérica por diferencias centrales
# J_ij = d g_i / d x_j (x)
# ---------------------------------------------
calcula.jacobiano <- function(G, x, h = sqrt(.Machine$double.eps)) {
  x <- as.numeric(x)
  m <- length(x)
  J <- matrix(0, nrow = m, ncol = m)
  for (j in seq_len(m)) {
    ej <- rep(0, m); ej[j] <- 1
    fp <- evaluar.G(G, x + h * ej)
    fm <- evaluar.G(G, x - h * ej)
    J[, j] <- (fp - fm) / (2 * h)
  }
  J
}
...
# ---------------------------------------------
# Newton para varias variables (vector)
# P0    : punto inicial
# tol   : tolerancia (máx. norma-infinito del salto)
# n.max : iteraciones máximas
# G     : lista de funciones (g1, ..., gn)
# ---------------------------------------------
newton.vv <- function(P0, tol = 1e-8, n.max = 50, G, desplegar = TRUE) {
  x.prev <- as.numeric(P0)
  for (i in seq_len(n.max)) {
    J   <- calcula.jacobiano(G, x.prev)
    Fx  <- evaluar.G(G, x.prev)
    # Resolver J * delta = F(x)  (mejor que invertir)
    ...
    if (desplegar) {
      cat(sprintf("Iteración %d: x = (%s)\n",
          i, paste(formatC(x.next, digits = 15, format = "g"), collapse = ", ")))
    }
    if (max(abs(x.next - x.prev)) < tol) return(x.next)
    x.prev <- x.next
  }
  message("Número máximo de iteraciones excedido")
  return(x.prev)
}
```
...
$$f(x, y) = x^2 + y^2 + 0.6y - 0.16 = 0, $$
$$g(x, y) = x^2 - y^2 + x - 1.6y - 0.14 = 0$$

```{r}
#| code-fold: true

f1 <- function(x1, x2) x1^2 + x2^2 + 0.6*x2 - 0.16
f2 <- function(x1, x2) x1^2 - x2^2 + x1 - 1.6*x2 - 0.14

F  <- list(f1, f2)
p0 <- c(0.6, 0.25)

sol <- newton.vv(p0, tol = 1e-10, n.max = 50, G = F, desplegar = TRUE)

cat("\nSolución aproximada:\n")

print(sol)

# --- Malla y evaluación para las curvas implícitas ---
# Rango (ajústalo si querés ver más área)
xr <- seq(-0.5, 1.5, length.out = 400)
yr <- seq(-0.5, 1.0, length.out = 400)

grid <- expand.grid(x = xr, y = yr)
grid$zf <- with(grid, f1(x, y))
grid$zg <- with(grid, f2(x, y))

# --- Gráfico: f(x,y)=0 (azul) y g(x,y)=0 (rojo) ---
ggplot(grid, aes(x, y)) +
  geom_contour(aes(z = zf, color = "f(x,y) = 0"), breaks = 0, linewidth = 1) +
  geom_contour(aes(z = zg, color = "g(x,y) = 0"), breaks = 0, linewidth = 1) +
  geom_point(data = data.frame(x = sol[1], y = sol[2]),
             aes(x, y), size = 3) +
  annotate("text", x = sol[1], y = sol[2], label = "  raíz",
           hjust = 0, vjust = -0.7, size = 4) +
  coord_fixed() +
  scale_color_manual(values = c("f(x,y) = 0" = "blue", "g(x,y) = 0" = "red"), name = NULL) +
  labs(title = "Intersección de f(x,y)=0 y g(x,y)=0",
       subtitle = sprintf("Solución ≈ (%.6f, %.6f)", sol[1], sol[2]),
       x = "x", y = "y") +
...
Es frecuente, en todas las disciplinas científicas, tener que resolver sistemas lineales de la forma
$$
A\mathbf{x}=\mathbf{b},
$$
donde $A$ es una matriz cuadrada de dimensión $n\times n$ cuyos elementos $a_{ij}$ son reales o complejos, y $\mathbf{x}$ y $\mathbf{b}$ son vectores columna de dimensión $n$, donde $\mathbf{x}$ es la incógnita y $\mathbf{b}$ un vector dado.
...
### Sistemas lineales y complejidad

La solución del sistema anterior existe y es única si y sólo si $A$ no es singular. En teoría, la solución puede calcularse utilizando las **fórmulas de Cramer**:
$$
x_i=\frac{\det(A_i)}{\det(A)},\qquad i=1,\ldots,n,
$$
donde $A_i$ es la matriz que se obtiene sustituyendo la $i$-ésima columna de $A$ por el vector $\mathbf{b}$, y donde $\det(A)$ denota el determinante de $A$.  
Si los $n + 1$ determinantes se calculan por el método usual, se requieren aproximadamente $3(n + 1)!$ operaciones.

Como de costumbre, se entiende por operación una suma, una resta, un producto o una división.


### Factorización LU

Sea $A \in \mathbb{R}^{n \times n}$.  
Supongamos que existen dos matrices, $L$ y $U$, respectivamente triangular inferior y superior, tales que:

$$
A = LU
$$

A esta igualdad se le llama **factorización** (o **descomposición**) LU de $A$.  
Si $A$ es invertible, entonces claramente $L$ y $U$ también lo son, y sus términos diagonales son no nulos.

En este caso, resolver $A\mathbf{x} = \mathbf{b}$ equivale a resolver dos sistemas triangulares:

$$
L\mathbf{y} = \mathbf{b}, \qquad U\mathbf{x} = \mathbf{y}.
$$

Estos dos sistemas son fáciles de resolver.  
En efecto, como $L$ es triangular inferior, la primera fila del sistema $L\mathbf{y} = \mathbf{b}$ tiene la forma:

$$
l_{11}y_1 = b_1,
$$

lo que da el valor de $y_1$, ya que $l_{11} \ne 0$.  
Sustituyendo este valor de $y_1$ en las $n - 1$ ecuaciones siguientes, se obtiene un nuevo sistema cuyas incógnitas son $y_2, \dots, y_n$, sobre las cuales se puede proceder de la misma manera.  
Avanzando ecuación por ecuación, se calculan así todas las incógnitas mediante el algoritmo denominado de **descenso** (o **sustitución progresiva**).

El algoritmo de descenso (o sustitución progresiva) se expresa mediante las siguientes fórmulas recursivas:

$$
\begin{cases}
y_1 = \dfrac{1}{l_{11}} b_1, \\[1em]
y_i = \dfrac{1}{l_{ii}} \left(b_i - \displaystyle\sum_{j=1}^{i-1} l_{ij} y_j \right), \quad i = 2, \dots, n.
\end{cases}
$$

Estas expresiones permiten calcular las componentes del vector $\mathbf{y}$ una por una, desde $y_1$ hasta $y_n$.

Evaluemos el número de operaciones requeridas por la ecuación (2.5).  
Para calcular la incógnita $y_i$, se realizan $i-1$ sumas, $i-1$ productos y una división.  
El número total de operaciones es, por tanto:

$$
\sum_{i=1}^n 1 + 2 \sum_{i=1}^n (i - 1) 
= 2 \sum_{i=1}^n i - n 
= n^2.
$$

::: {.callout-tip collapse="true" title="Ejemplo"}
Resolver el sistema $L\mathbf{y} = \mathbf{b}$ cuando $L$ es una matriz triangular inferior de orden 3.
...
Deseamos resolver el sistema $L\mathbf{y} = \mathbf{b}$ mediante el **algoritmo de descenso** (o **sustitución progresiva**):

$$
y_1 = \frac{b_1}{l_{11}} = \frac{4}{2} = 2
$$
(1 división)

$$
y_2 = \frac{b_2 - l_{21}y_1}{l_{22}} = \frac{5 - 3\cdot2}{1} = \frac{5 - 6}{1} = -1
$$
(1 producto, 1 resta, 1 división)

$$
y_3 = \frac{b_3 - l_{31}y_1 - l_{32}y_2}{l_{33}} 
= \frac{-1 - 1\cdot2 - (-4)\cdot(-1)}{5} 
= \frac{-1 - 2 - 4}{5} 
= -\frac{7}{5}
$$
(2 productos, 2 restas, 1 división)
...
El número total de operaciones elementales (sumas/restas, productos y divisiones) es:

$$
1 + 3 + 5 = 9 = 3^2,
$$

lo cual coincide con la fórmula general:

$$
\sum_{i=1}^n 1 + 2\sum_{i=1}^n (i - 1) = n^2.
$$

Podemos resolver el sistema $U\mathbf{x}=\mathbf{y}$ de manera similar.  
Esta vez se comienza por determinar $x_n$ y luego, paso a paso hacia atrás, las demás incógnitas $x_i$ para $i=n-1,\dots,1$:

$$
\left\{
\begin{aligned}
x_n &= \frac{1}{u_{nn}}\,y_n,\\[4pt]
x_i &= \frac{1}{u_{ii}}\!\left(\,y_i-\sum_{j=i+1}^{n} u_{ij}\,x_j\right), \qquad i=n-1,\dots,1 .
\end{aligned}
\right.
\tag{2.6}
$$

Este procedimiento es el **algoritmo de ascenso** (sustitución regresiva) y necesita también $n^2$ operaciones.

:::

::: {.callout-tip collapse="true" title="Ejemplo"}

Resolver $U\mathbf{x}=\mathbf{y}$ por sustitución regresiva (triangular superior, $n=3$)
...
\mathbf{y}=\begin{bmatrix}10\\-2\\15\end{bmatrix}.
$$

Procedemos de atrás hacia adelante:

1) $5x_3=15 \Rightarrow x_3=\dfrac{15}{5}=3$.

2) $2x_2-4x_3=-2 \Rightarrow 2x_2-4\cdot3=-2 \Rightarrow 2x_2=10 \Rightarrow x_2=\dfrac{10}{2}=5$.

3) $3x_1-x_2+2x_3=10 \Rightarrow 3x_1-5+2\cdot3=10 \Rightarrow 3x_1+1=10
\Rightarrow 3x_1=9 \Rightarrow x_1=\dfrac{9}{3}=3$.

Por lo tanto,
$$
\mathbf{x}=\begin{bmatrix}3\\5\\3\end{bmatrix}.
$$

**Conteo de operaciones.** Para $n=3$ se realizan $1+3+5=9=3^2$ operaciones elementales (sumas/restas, productos y divisiones).
:::

::: {.callout-tip collapse="true" title="Ejemplo"}

Resolver $A\mathbf{x}=\mathbf{b}$ con la factorización $A=LU$ (descenso + ascenso)
...
Sea
$$
\mathbf{b}=\begin{bmatrix}20\\[2pt]28\\[2pt]93\end{bmatrix}.
$$

** Paso 1: Descenso $L\mathbf{y}=\mathbf{b}$ **
Calculamos $\mathbf{y}$ componente a componente (sustitución progresiva):
$$
y_1=\frac{b_1}{\;l_{11}\;}=\frac{20}{2}=10,
$$
$$
y_2=\frac{\,b_2-l_{21}y_1\,}{\;l_{22}\;}=\frac{28-3\cdot10}{1}=28-30=-2,
$$
$$
y_3=\frac{\,b_3-l_{31}y_1-l_{32}y_2\,}{\;l_{33}\;}
=\frac{\,93-1\cdot10-(-4)\cdot(-2)\,}{5}
=\frac{\,93-10-8\,}{5}=\frac{75}{5}=15.
$$

Por tanto,
$$
\mathbf{y}=\begin{bmatrix}10\\[2pt]-2\\[2pt]15\end{bmatrix}.
$$

**Paso 2: Ascenso $U\mathbf{x}=\mathbf{y}$**
Resolvemos de atrás hacia adelante (sustitución regresiva):
$$
x_3=\frac{y_3}{\;u_{33}\;}=\frac{15}{5}=3,
$$
$$
x_2=\frac{\,y_2-u_{23}x_3\,}{\;u_{22}\;}
=\frac{-2-(-4)\cdot3}{2}
=\frac{-2+12}{2}=\frac{10}{2}=5,
$$
$$
x_1=\frac{\,y_1-u_{12}x_2-u_{13}x_3\,}{\;u_{11}\;}
=\frac{\,10-(-1)\cdot5-2\cdot3\,}{3}
=\frac{10+5-6}{3}=\frac{9}{3}=3.
$$

**Solución**
$$
\mathbf{x}=\begin{bmatrix}3\\[2pt]5\\[2pt]3\end{bmatrix}.
$$
...
3&-9&43
\end{bmatrix}
\begin{bmatrix}3\\[2pt]5\\[2pt]3\end{bmatrix}
=
\begin{bmatrix}20\\[2pt]28\\[2pt]93\end{bmatrix}
=\mathbf{b}.
$$

Con esto se confirma que la solución obtenida por el esquema $A=LU$ (descenso + ascenso) es correcta.
:::

::: {.callout-tip collapse="true" title="Ejemplo"}
Factorización $LU$ para $A\in\mathbb{R}^{2\times2}$

**Solución**

Escribamos la relación $A=LU$ para una matriz cualquiera $A\in\mathbb{R}^{2\times2}$:
...
Los seis elementos desconocidos de $L$ y $U$ deben satisfacer las siguientes ecuaciones (no lineales):

$$
\begin{aligned}
(e_1)\;& l_{11}u_{11}=a_{11}, 
\qquad
(e_2)\; l_{11}u_{12}=a_{12},\\[2pt]
(e_3)\;& l_{21}u_{11}=a_{21},
\qquad
(e_4)\; l_{21}u_{12}+l_{22}u_{22}=a_{22}.
\end{aligned}
$$

El sistema anterior es **subdeterminado** (hay menos ecuaciones que incógnitas). Podemos completarlo fijando arbitrariamente los coeficientes diagonales de $L$; por ejemplo, tomamos $l_{11}=1$ y $l_{22}=1$.

Con esta elección puede resolverse así:
1) Primero se determinan $u_{11}$ y $u_{12}$ (primera fila de $U$) usando $(e_1)$ y $(e_2)$.
2) Si $u_{11}\neq 0$, de $(e_3)$ se obtiene $l_{21}$ (primer elemento de la segunda fila de $L$, con $l_{11}$ ya conocido).
3) Finalmente, usando $(e_4)$, se determina el único elemento no nulo $u_{22}$ de la segunda fila de $U$.
:::

::: {.callout-tip collapse="true" title="Ejemplo"}

Retomemos los cálculos en el caso de una matriz $3\times 3$. Para determinar los 12 coeficientes desconocidos de $L$ y $U$ en $A=LU$, se dispone de las 9 ecuaciones
$$
\begin{aligned}
(e_1)\;& l_{11}u_{11}=a_{11}, 
& (e_2)\;& l_{11}u_{12}=a_{12}, 
& (e_3)\;& l_{11}u_{13}=a_{13},\\
(e_4)\;& l_{21}u_{11}=a_{21},
& (e_5)\;& l_{21}u_{12}+l_{22}u_{22}=a_{22},
& (e_6)\;& l_{21}u_{13}+l_{22}u_{23}=a_{23},\\
(e_7)\;& l_{31}u_{11}=a_{31},
& (e_8)\;& l_{31}u_{12}+l_{32}u_{22}=a_{32},
& (e_9)\;& l_{31}u_{13}+l_{32}u_{23}+l_{33}u_{33}=a_{33}.
\end{aligned}
$$

El sistema es **subdeterminado**. Se completa imponiendo $l_{ii}=1$ para $i=1,2,3$.

- Con $(e_1)$, $(e_2)$, $(e_3)$ se obtienen $u_{11},u_{12},u_{13}$ (primera fila de $U$).
- Usando $(e_4)$ y $(e_7)$ se determinan $l_{21}$ y $l_{31}$ (primera columna de $L$).
- Con $(e_5)$ y $(e_6)$ se calculan $u_{22},u_{23}$ (segunda fila de $U$).
- Usando $(e_8)$ se determina $l_{32}$ (segunda columna de $L$).
- Finalmente, $(e_9)$ determina $u_{33}$ (última fila/columna de $U$).

**Caso general $n\times n$.**

1. Los elementos de $L$ y $U$ satisfacen, para $i,j=1,\dots,n$,
\begin{equation}
\sum_{r=1}^{\min(i,j)} l_{ir}u_{rj} = a_{ij}.
\tag{2.7}
\end{equation}
2. El sistema $(2.7)$ es **subdeterminado**; en efecto, hay $n^2$ ecuaciones y $n^2 + n$ incógnitas.  
Por tanto, la factorización $LU$ no puede ser única; de hecho, existe una **infinidad** de matrices $L$ y $U$ que satisfacen $(2.7)$.

3. Fijando el valor $1$ para los $n$ elementos diagonales de $L$, el sistema $(2.7)$ se convierte en un sistema determinado que puede resolverse mediante el *Algoritmo de Gauss*.  

Sea $A^{(1)} = A$, es decir, $a_{ij}^{(1)} = a_{ij}$ para $i, j = 1, \dots, n$:

\begin{equation}
\begin{cases}
\text{para } k = 1, \dots, n-1, \\[3pt]
\quad \text{para } i = k + 1, \dots, n, \\[3pt]
\qquad l_{ik} = \dfrac{a_{ik}^{(k)}}{a_{kk}^{(k)}}, \\[8pt]
\quad \text{para } j = k + 1, \dots, n, \\[3pt]
\qquad a_{ij}^{(k+1)} = a_{ij}^{(k)} - l_{ik} a_{kj}^{(k)}.
\end{cases}
\tag{2.8}
\end{equation}

Los términos $a_{kk}^{(k)}$, llamados pivotes, deben ser todos no nulos.  
Para $k = 1, \dots, n-1$, la matriz $A^{(k+1)} = (a_{ij}^{(k+1)})$ tiene $n - k$ filas y columnas.

:::

::: {.callout-important collapse="true" title="Observación"}

No es necesario almacenar todas las matrices $A^{(k)}$ durante el algoritmo $(2.8)$.  
De hecho, es posible sobrescribir los últimos $(n - k) \times (n - k)$ elementos de la matriz original $A$ con los $(n - k) \times (n - k)$ elementos de $A^{(k+1)}$.

Además, en la etapa $k$, los elementos situados **bajo la diagonal** de la $k$-ésima columna no afectan a la matriz final $U$; por tanto, pueden reemplazarse por los coeficientes de la $k$-ésima columna de $L$ (los **multiplicadores**).  
Esto es precisamente lo que se realiza en la implementación de `LU.html` en **R**.

En la etapa $k$ del algoritmo, los elementos almacenados en lugar de los coeficientes originales de $A$ son:

$$
\begin{bmatrix}
a^{(1)}_{11} & a^{(1)}_{12} & \cdots & a^{(1)}_{1n} \\
l_{21} & a^{(2)}_{22} & & a^{(2)}_{2n} \\
\vdots & \ddots & \ddots & \vdots \\
l_{k1} & \cdots & l_{k,k-1} & 
\boxed{
\begin{matrix}
a^{(k)}_{kk} & \cdots & a^{(k)}_{kn} \\
\vdots & \ddots & \vdots \\
a^{(k)}_{nk} & \cdots & a^{(k)}_{nn}
\end{matrix}
}
\end{bmatrix}
$$

donde la submatriz en el recuadro corresponde a $A^{(k)}$.

Así, el algoritmo puede implementarse **almacenando una sola matriz**, inicializada con $A$, que se actualiza en cada iteración $k \ge 2$ sobrescribiendo los nuevos términos $a^{(k)}_{ij}$ (para $i, j \ge k + 1$) y los multiplicadores $l_{ik}$ (para $i \ge k + 1$).  Nótese que **no es necesario almacenar los elementos diagonales** $l_{ii}$, ya que se sabe que todos valen $1$.

:::

::: {.callout-important collapse="true" title="Observación: Cálculo de un determinante"}

La factorización $LU$ permite calcular el determinante de $A$ con aproximadamente $\mathcal{O}(n^2)$ operaciones (si ya se tienen calculadas $L$ y $U$).  
Basta con notar que:
...
::: {.callout-tip collapse="true" title="Ejemplo 7"}

Supongamos que se desea resolver el sistema $A\mathbf{x} = \mathbf{b}$ con:
...
independientemente del valor de $\varepsilon$.

Sea ahora $\varepsilon = 1$.  
La factorización de Gauss de $A$ obtenida con el programa en **R** es:
...
Si se toma $\varepsilon = 0$, no es posible efectuar la factorización de Gauss  
(aunque $A$ no sea singular, i.e., invertible), ya que el algoritmo $(2.8)$ provocaría una división por cero.

El ejemplo anterior muestra que la factorización de Gauss $A = LU$ no existe para toda matriz invertible $A$.  
Sin embargo, puede establecerse el siguiente resultado:

:::

::: {.callout-note title="Proposición"}

Sea $A \in \mathbb{R}^{n \times n}$ una matriz cualquiera.  
La factorización de Gauss existe y es **única si y sólo si** las submatrices principales $A_i$ de $A$, de orden $i = 1, \dots, n-1$ (aquellas obtenidas al restringir $A$ a sus $i$ primeras filas y columnas), no son singulares (i.e., son invertibles).  

En otras palabras, si los **menores principales**, es decir, los determinantes de las submatrices principales, son todos no nulos.

:::

Con mayor detalle, se tiene el siguiente resultado:

::: {.callout-note title="Proposición"}

Sea $A \in \mathbb{F}^{n \times n}$, con $\mathbb{F} = \mathbb{R}$ o $\mathbb{C}$.  
Las siguientes afirmaciones son equivalentes:

1. Todos los **menores principales líderes** son no nulos, es decir,  
   $\det(A_{1:k,\,1:k}) \ne 0$ para $k = 1, \dots, n$.

2. Existe una factorización $A = LU$ con $L$ triangular inferior **unitaria** (diagonal de unos)  
   y $U$ triangular superior con **diagonal no nula**.

Además, cuando existen, $L$ y $U$ son **únicas**.

::: {.callout-caution collapse="true" title="Prueba"}

**$(2 \Rightarrow 1)$**.  
Suponga $A = LU$ con $L$ triangular inferior unitaria y $U$ triangular superior.  
Para cada $k$ el menor principal líder de orden $k$ se escribe como
$$
A_{1:k,\,1:k}=L_{1:k,\,1:k}\,U_{1:k,\,1:k},
$$
donde $L_{1:k,\,1:k}$ es triangular inferior unitaria y $U_{1:k,\,1:k}$ triangular superior.  
Tomando determinantes,
$$
\det(A_{1:k,\,1:k})
=\det(L_{1:k,\,1:k})\,\det(U_{1:k,\,1:k})
=\Big(\prod_{i=1}^k 1\Big)\Big(\prod_{i=1}^k u_{ii}\Big)
=\prod_{i=1}^k u_{ii}.
$$
Como $u_{ii}\neq 0$ (la diagonal de una triangular superior invertible es no nula), se concluye $\det(A_{1:k,\,1:k})\neq 0$ para todo $k$.

**$(1 \Rightarrow 2)$**.  
Procedemos por inducción en $n$ (equivalentemente, por eliminación de Gauss sin permutaciones).

- Para $n=1$ es trivial.

- Suponga el enunciado cierto hasta tamaño $n-1$ y sea $A\in\mathbb{F}^{n\times n}$ con $\det(A_{1:k,\,1:k})\neq 0$ para $k=1,\dots,n$.  
En particular $a_{11}\neq 0$. Escriba $A$ en bloques
$$
A=\begin{bmatrix}
a_{11} & a_{12}^t\\
a_{21} & A_{22}
...
$$
Defina los **multiplicadores** $\ell=a_{21}/a_{11}\in\mathbb{F}^{n-1}$ y forme el **complemento de Schur**
$$
S \;=\; A_{22}-\ell\, a_{12}^t .
$$
Entonces
...
\end{bmatrix}.
$$
Como $L_1=\begin{bmatrix}1&0\\ \ell&I\end{bmatrix}$ es triangular inferior unitaria, basta factorizar el bloque superior derecho.  
Además, para cada $k=2,\dots,n$, el menor principal líder de orden $k$ de $A$ se descompone como
$$
\det(A_{1:k,\,1:k}) \;=\; a_{11}\,\det\!\big(S_{1:(k-1),\,1:(k-1)}\big),
$$


Dado que por hipótesis $\det(A_{1:k,\,1:k}) \neq 0$ y $a_{11} \neq 0$, concluimos que  
$\det(S_{1:(k-1),\,1:(k-1)}) \neq 0$ para toda orden $k - 1 = 1, \dots, n - 1$.  

Por hipótesis inductiva, $S$ admite una factorización $S = L_2 U_2$ con $L_2$ unitaria inferior y $U_2$ superior.  
Poniendo todo junto,
...
donde $L$ es triangular inferior unitaria y $U$ triangular superior.  
Esto prueba la **existencia**.

**(Unicidad).**  
Suponga $A = L_1 U_1 = L_2 U_2$, con $L_1, L_2$ unitarias inferiores y $U_1, U_2$ superiores.  
Entonces:
...
El miembro izquierdo es triangular inferior unitaria y el derecho triangular superior.  
La única matriz que es simultáneamente triangular superior e inferior y con diagonal de unos es la identidad.  
Por tanto, $L_1 = L_2$ y $U_1 = U_2$.  
...
Volviendo al **Ejemplo 7**, se observa que cuando $\varepsilon = 0$, la segunda submatriz principal $A_2$ de $A$ es singular.

Podemos identificar clases particulares de matrices para las cuales se satisfacen las hipótesis de la **Proposición 1** o **2**.  

Por ejemplo:

1. **Las matrices con diagonal estrictamente dominante.**

Una matriz se dice de **diagonal dominante por filas** si

$$
|a_{ii}| \ge \sum_{\substack{j=1 \\ j \ne i}}^n |a_{ij}|, \quad i = 1, \dots, n,
$$

y **por columnas** si

$$
|a_{ii}| \ge \sum_{\substack{j=1 \\ j \ne i}}^n |a_{ji}|, \quad i = 1, \dots, n.
$$

Cuando se puede reemplazar el signo “$\ge$” por “$>$” en las desigualdades anteriores,  
la matriz $A$ se denomina de **diagonal estrictamente dominante** (ya sea por filas o por columnas).

2. **Las matrices reales simétricas definidas positivas.**
...
::: {.callout-tip collapse="true" title="Ejemplo: Matriz con diagonal estrictamente dominante por filas"}
...
Por tanto, $A$ es de **diagonal estrictamente dominante por filas**  
y cumple las condiciones de la **Proposición 1**;  
en consecuencia, posee una factorización $A = LU$ sin necesidad de pivoteo  
(más adelante se presentará el algoritmo con pivoteo).

:::

::: {.callout-tip collapse="true" title="Ejemplo: Matriz simétrica definida positiva"}
...
Es simétrica ($B = B^T$). Verifiquemos los menores principales:
...
por lo que $B$ es **definida positiva**.  
Además, admite la **factorización de Cholesky** $B = LL^T$, con:
...
Si $A \in \mathbb{R}^{n \times n}$ es simétrica definida positiva, puede construirse una factorización particular:

$$
A = R^T R,
$$

donde $R$ es una matriz triangular superior con elementos diagonales positivos.  
Esta descomposición se denomina **factorización de Cholesky** y requiere aproximadamente $\tfrac{n^3}{3}$ operaciones (la mitad de las necesarias para la factorización $LU$ de Gauss).

Además, gracias a la simetría, sólo es necesario almacenar la parte superior de $A$,  
y los coeficientes de $R$ pueden colocarse en el mismo espacio de memoria.

Los coeficientes de $R$ pueden calcularse mediante el siguiente algoritmo:

Se define $r_{11} = \sqrt{a_{11}}$, y para $i = 2, \dots, n$, se tiene:

$$
\begin{cases}
r_{ji} = \dfrac{1}{r_{jj}} \left(a_{ij} - \displaystyle\sum_{k=1}^{j-1} r_{ki} r_{kj}\right), & j = 1, \dots, i-1, \\
\\
r_{ii} = \sqrt{a_{ii} - \displaystyle\sum_{k=1}^{i-1} r_{ki}^2}.
\end{cases}
\tag{2.10}
$$
...
::: {.callout-tip collapse="true" title="Ejemplo: Cálculo de la factorización de Cholesky"}
...
por lo tanto $B$ es **definida positiva**.  
Calcularemos su factorización de Cholesky $B = R^T R$,  
con $R$ triangular superior (diagonal positiva), siguiendo (2.10).
...
$$
r_{12} = \frac{1}{r_{11}} (a_{12} - \sum_{k=1}^{0} r_{k2} r_{k1}) = \frac{a_{12}}{r_{11}} = \frac{2}{2} = 1,
$$

$$
r_{22} = \sqrt{a_{22} - \sum_{k=1}^{1} r_{k2}^2} = \sqrt{5 - r_{12}^2} = \sqrt{5 - 1} = 2.
$$

**Paso $i = 3$**

$$
r_{13} = \frac{1}{r_{11}} (a_{13} - \sum_{k=1}^{0} r_{k3} r_{k1}) = \frac{a_{13}}{r_{11}} = \frac{-2}{2} = -1,
$$

$$
r_{23} = \frac{1}{r_{22}} (a_{23} - \sum_{k=1}^{1} r_{k3} r_{k2}) = \frac{1 - r_{13} r_{12}}{2} = \frac{1 - (-1)\cdot1}{2} = \frac{2}{2} = 1,
$$

$$
r_{33} = \sqrt{a_{33} - \sum_{k=1}^{2} r_{k3}^2} = \sqrt{6 - (r_{13}^2 + r_{23}^2)} = \sqrt{6 - (1 + 1)} = 2.
$$
...
Esto verifica la factorización de Cholesky $B = R^t R$.
